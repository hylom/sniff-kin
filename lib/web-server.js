...
export class WebServer extends Pluggable {
  constructor(config) {
    ...
    // enable webserver access log if DEBUG mode
  ...
  getLocalBaseUrl() {
    const bindAddress = this.app.get('bindAddress');
    let host = this.app.get('exposedAddress');
  ...
  listen(option) {
    ...
    if (option.bindAddress) {
      this.app.set('bindAddress', option.bindAddress);
    }
