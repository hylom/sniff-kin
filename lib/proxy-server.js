...
import { ProxySession, ConnectSession } from './session.js';
...
export class ProxyServer extends Pluggable {
  ...
  listen(option) {
    ...
    if (option.bindAddress) {
      this.config.host = option.bindAddress;
    }
    ...
    proxy.onConnect((req, clientSocket, head, callback) => {
      ...
      const serverSocket = net.connect(connOpt, () => {
        ...
        const session =  new ConnectSession(req, clientSocket, serverSocket, head);
        this.executeHandler('connect', session);
      });
