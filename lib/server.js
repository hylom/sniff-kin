...
export class Server {
  ...
  startProxyServer(option) {
    this.proxyServer.addHandler('listen', () => {
      logger.info(`proxy server started on ${option.bindAddress}:${this.config.proxy.port}`);
    });
  ...
  startWebServer(option) {
    this.webServer.addHandler('listen', () => {
      logger.info(`web server started on ${option.bindAddress}:${this.config.web.port}`);
    });
  ...
  async start() {
    ...
    logger.info(`exposed IP Address is ${exposedAddress}...`);
